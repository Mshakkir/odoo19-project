...
class AccountPayment(models.Model):
    ...
    # -------------------------------------------------------
    # Allocation history – vendor (purchase) side
    # -------------------------------------------------------
    bill_allocation_history_ids = fields.One2many(
        'bill.allocation.history', 'payment_id',
        string='Bill Allocations', readonly=True)
    bill_allocation_count = fields.Integer(
        string='Bill Allocations', compute='_compute_bill_allocation_count')
    ...
    def action_view_bill_allocation_history(self):
        """Open wizard to display bill allocation details (vendor/purchase side)"""
        self.ensure_one()
        ...
        if not self.bill_allocation_history_ids:
            return {
                ...
                'params': {
                    ...
                    'message': _(
                        'This payment does not have bill allocation history. '
                        'This feature only works for payments created through '
                        'the "Against Payments" wizard.'),
                    'type': 'info',
                }
            }

        # Create display wizard record
        wizard = self.env['bill.allocation.display.wizard'].create({
            'payment_id': self.id,
        })

        # Populate wizard lines from permanent history
    ...
    # -------------------------------------------------------
    # Partner propagation helpers (shared by both modules)
    # -------------------------------------------------------
    @api.model_create_multi
    def create(self, vals_list):
        """Override create to ensure partner context is passed"""
        ...
        for payment in payments:
            _logger.info(
                f"Created payment {payment.name} for partner "
                f"{payment.partner_id.name if payment.partner_id else 'N/A'}")
            ...
            if payment.move_id and payment.partner_id:
                if not payment.move_id.partner_id:
                    try:
                        payment.move_id.write({'partner_id': payment.partner_id.id})
                        _logger.info(
                            f"Set partner {payment.partner_id.name} on move {payment.move_id.name}")
                    except Exception as e:
                        _logger.warning(
                            f"Could not set partner on move {payment.move_id.name}: {str(e)}")
                ...
                for line in payment.move_id.line_ids:
                    if not line.partner_id:
                        try:
                            line.write({'partner_id': payment.partner_id.id})
                            _logger.info(
                                f"Set partner {payment.partner_id.name} on move line "
                                f"for account {line.account_id.code}")
                        except Exception as e:
                            _logger.warning(f"Could not set partner on line: {str(e)}")
    ...
    def action_post(self):
        ...
        for payment in self:
            if payment.move_id and payment.partner_id:
                lines_without_partner = payment.move_id.line_ids.filtered(
                    lambda l: not l.partner_id)

                if lines_without_partner:
                    _logger.warning(
                        f"Found {len(lines_without_partner)} lines without partner "
                        f"after posting payment {payment.name}")

                    for line in lines_without_partner:
                        try:
                            line.sudo().write({'partner_id': payment.partner_id.id})
                            _logger.info(
                                f"POST-FIX: Set partner {payment.partner_id.name} on line "
                                f"{line.id} for account {line.account_id.code}")
                        except Exception as e:
                            _logger.error(
                                f"CRITICAL: Could not set partner on line {line.id}: {str(e)}")

                for line in payment.move_id.line_ids:
                    _logger.info(
                        f"Final line status - Account: {line.account_id.code}, "
                        f"Partner: {line.partner_id.name if line.partner_id else 'MISSING'}, "

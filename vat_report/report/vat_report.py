...
class VATReport(models.AbstractModel):
    _name = "report.vat_report.vat_report_pdf"
    ...
    def _get_tax_data(self, tax_ids):
        ...
        for tax in taxes:
            ...
            for repartition_line in tax.invoice_repartition_line_ids:
                tags_ids.extend(repartition_line.tag_ids.ids)
    ...
    def _get_tax_report_domain(self, company_id, date_from, date_to, only_posted_moves):
        ...
        # Tax exigibility check
        domain += ["|", ("tax_exigible", "=", True), ("tax_exigible", "=", False)]
    ...
    def _get_vat_report_data(self, company_id, date_from, date_to, only_posted_moves):
        ...
        ml_fields = ["id", "tax_base_amount", "balance", "tax_line_id", "tax_ids"]
            ...
            tax_group_data.update(
                {
                    tax_group.id: {
                        "id": tax_group.id,
                        "name": tax_group.name,
                        "code": str(tax_group.sequence) if tax_group.sequence else "",
                    }
                }
            )

...
patch(ListController.prototype, {
    ...
    attachFilterEvents(fromId, toId, warehouseId, customerId, salespersonId, documentNumberId, totalAmountId, customerRefId, poNumberId, shippingRefId, applyId, clearId, actionName, isSaleOrder, isInvoice) {
        ...
        if (filterContainer) {
            // Add keydown listeners to all inputs and selects for Enter
            const allInputs = filterContainer.querySelectorAll('.filter-input, select');
            allInputs.forEach(input => {
                input.addEventListener('keydown', (e) => {
                    if (e.key === 'Enter') {
            ...
            // Global keyboard listener for backspace to clear filters
            document.addEventListener('keydown', (e) => {
                // Check if backspace is pressed and we're in the filter area
                if (e.key === 'Backspace') {
                    const activeElement = document.activeElement;
                    const isInFilterContainer = filterContainer.contains(activeElement);

                    // Only trigger clear if:
                    // 1. Active element is in filter container AND
                    // 2. Active element is NOT an input/textarea/select OR
                    // 3. Active element is an input but it's empty
                    if (isInFilterContainer) {
                        const isInputField = activeElement.matches('input, textarea, select');

                        if (!isInputField || (activeElement.matches('input') && activeElement.value === '')) {
                            e.preventDefault();
                            clearFilters();
                        }
                    }

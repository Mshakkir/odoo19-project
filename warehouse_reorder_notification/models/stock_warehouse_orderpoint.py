...
class StockWarehouseOrderpoint(models.Model):
    ...
    has_active_notification = fields.Boolean(
        ...
        store=True,
        help='Whether this orderpoint currently has an active notification'
    )
    ...
    ], string='Stock Status', compute='_compute_notification_status', store=True)

    @api.depends('product_id', 'location_id', 'product_min_qty', 'product_max_qty')
    def _compute_notification_status(self):
        """Compute current stock status and active notification state"""
        for orderpoint in self:
            if not orderpoint.product_id or not orderpoint.location_id:
                orderpoint.stock_status = 'ok'
                orderpoint.has_active_notification = False
                continue
            ...
            if qty_available < orderpoint.product_min_qty:
                orderpoint.stock_status = 'below_min'
            elif qty_available > orderpoint.product_max_qty:
                orderpoint.stock_status = 'above_max'
            else:
                orderpoint.stock_status = 'ok'
    ...
    def _auto_close_notifications_if_stock_ok(self):
        """Auto-close notifications when stock reaches acceptable levels"""
        self.ensure_one()
        ...
        if qty_available >= self.product_min_qty and qty_available <= self.product_max_qty:
            ...
            if activities:
                ...
                self._send_stock_ok_notification(qty_available)

    def _send_stock_ok_notification(self, current_qty):
        """Send notification that stock is now OK"""
        self.ensure_one()
        ...
        message = f"""
        <div style="padding: 10px; border-left: 4px solid #28a745; background-color: #d4edda;">
            ...
            <table style="width: 100%; font-size: 13px;">
                <tr>
                    ...
                    <td style="padding: 3px;">{self.product_id.default_code and '[' + self.product_id.default_code + '] ' or ''}{self.product_id.name}</td>
                </tr>
                <tr>
                    <td style="padding: 3px; font-weight: bold;">Warehouse:</td>
                    <td style="padding: 3px;">{warehouse.name}</td>
                </tr>
                <tr style="background-color: #d4edda;">
                    <td style="padding: 3px; font-weight: bold;">Current Qty:</td>
                    <td style="padding: 3px; font-weight: bold; color: #28a745;">{current_qty:.2f} {self.product_id.uom_id.name}</td>
                </tr>
                <tr>
                    <td style="padding: 3px; font-weight: bold;">Min Qty:</td>
                    <td style="padding: 3px;">{self.product_min_qty:.2f} {self.product_id.uom_id.name}</td>
                </tr>
            </table>
            <div style="margin-top: 10px; padding: 8px; background-color: #d4edda; border-radius: 3px;">
                <strong>Stock levels are now within acceptable range. Reorder notification closed automatically.</strong>
            </div>
        </div>
        """
        ...
        for user in users_to_notify:
            notifications.append([
                ...
                'mail.activity/updated',
                {
                    ...
                    'title': f'✅ [{warehouse.name}] Stock Replenished: {self.product_id.name}',
                    'message': 'Stock is now within acceptable range.',
                }
            ])
    ...
    def action_close_notification(self):
        ...
        if activities:
            ...
            return {
                ...
                'params': {
                    'title': _('✅ Notification Closed'),
                    'message': _('All active notifications closed successfully.'),
                    'type': 'success',
                    'sticky': False,
                }
            }
            ...
            return {
                ...
                'tag': 'display_notification',
                'params': {
                    'title': _('No Active Notifications'),
                    'message': _('There are no active notifications to close.'),
                    'type': 'info',
                    'sticky': False,
                }
            }
        ...
        for warehouse in warehouses:
            ...
            for orderpoint in orderpoints:
                try:
                    ...
                    # Check if stock is OK now - auto-close notifications
                    if qty_available >= orderpoint.product_min_qty and qty_available <= orderpoint.product_max_qty:

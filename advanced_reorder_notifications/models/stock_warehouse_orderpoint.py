# Diagnostic code to check what's happening
# Use this in a Scheduled Action to debug

import logging

_logger = logging.getLogger(__name__)

# Get all reordering rules
orderpoints = env['stock.warehouse.orderpoint'].search([])

_logger.info("=" * 80)
_logger.info("REORDER RULES DIAGNOSTIC")
_logger.info("=" * 80)

for op in orderpoints:
    _logger.info(f"\nProduct: {op.product_id.name}")
    _logger.info(f"Location: {op.location_id.complete_name}")

    # Check different quantity methods
    product = op.product_id.with_context(location=op.location_id.id)

    _logger.info(f"  qty_available (forecast): {product.qty_available}")
    _logger.info(f"  free_qty: {product.free_qty if hasattr(product, 'free_qty') else 'N/A'}")

    # Try to get actual on hand
    quants = env['stock.quant'].search([
        ('product_id', '=', op.product_id.id),
        ('location_id', '=', op.location_id.id)
    ])
    actual_qty = sum(quants.mapped('quantity'))
    _logger.info(f"  actual qty (from quants): {actual_qty}")

    _logger.info(f"  Min qty: {op.product_min_qty}")
    _logger.info(f"  Max qty: {op.product_max_qty}")

    # Check notification status
    _logger.info(
        f"  notify_on_reorder: {op.notify_on_reorder if hasattr(op, 'notify_on_reorder') else 'Field not exists'}")
    _logger.info(
        f"  notification_sent: {op.notification_sent if hasattr(op, 'notification_sent') else 'Field not exists'}")

    # Determine if should notify
    should_notify = actual_qty <= op.product_min_qty
    _logger.info(f"  >>> SHOULD NOTIFY: {should_notify}")

    if should_notify:
        _logger.info(f"  >>> ALERT: Stock is below minimum!")

        # Check if method exists
        if hasattr(op, 'send_reorder_notification'):
            try:
                # Try to send notification
                op.send_reorder_notification()
                _logger.info(f"  >>> Notification method executed!")
            except Exception as e:
                _logger.error(f"  >>> Error calling notification: {e}")
        else:
            _logger.error(f"  >>> Method 'send_reorder_notification' does not exist!")

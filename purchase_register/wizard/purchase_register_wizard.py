...
class PurchaseRegisterWizard(models.TransientModel):
    ...
    def _get_manual_rate(self, record, company):
        """
        Get manual currency rate from PO or Invoice.
        Returns SAR per 1 foreign currency unit (e.g. 4.0 means 1 USD = 4 SAR).
        Falls back to system rate if manual rate not set.
        """
        company_currency = company.currency_id
        record_currency = getattr(record, 'currency_id', company_currency)

        if not record_currency or record_currency == company_currency:
            return 1.0

        # Try manual_currency_rate from purchase_order_awb / purchase_bill_form_modified
        if hasattr(record, 'manual_currency_rate') and record.manual_currency_rate:
            return record.manual_currency_rate

        # Fallback: system rate
        rate_date = None
        if hasattr(record, 'date_order') and record.date_order:
            rate_date = record.date_order.date() if hasattr(record.date_order, 'date') else record.date_order
        ...
        try:
            rates = record_currency._get_rates(company, rate_date)
            unit_per_sar = rates.get(record_currency.id, 1.0)
            return (1.0 / unit_per_sar) if unit_per_sar else 1.0
        except Exception:
            return 1.0

    def _get_invoice_cash_rounding_amount(self, invoice):
        """
        Get the actual cash rounding amount from invoice.
        This handles the invoice_cash_rounding_id field properly.
        Works for both customer invoices and vendor bills.
        """
        rounding_amount = 0.0

        # Check if invoice has cash rounding applied
        if hasattr(invoice, 'invoice_cash_rounding_id') and invoice.invoice_cash_rounding_id:
            # Look for the rounding line in the invoice lines
            for line in invoice.line_ids:
                if line.display_type == 'rounding':
                    rounding_amount = line.balance
                    break

            # Alternative: Calculate from amount_total vs computed total
            if rounding_amount == 0.0:
                computed_total = invoice.amount_untaxed + invoice.amount_tax
                rounding_amount = invoice.amount_total - computed_total

        # Fallback to amount_round field if it exists (custom field)
        elif hasattr(invoice, 'amount_round'):
            rounding_amount = invoice.amount_round
    ...
    def _get_purchase_order_data(self):
        ...
        for po in purchase_orders:
            ...
            for line in po.order_line:
                ...
                # Get manual currency rate for this PO
                rate = self._get_manual_rate(po, self.company_id)
                ...
                po_data.append({
                    ...
                    'unit_price': line.price_unit * rate,
                    'subtotal': line.price_subtotal * rate,
                    'trade_discount': trade_discount * rate,
                    'addin_discount': addin_discount * rate,
                    'addin_cost': addin_cost * rate,
                    'tax_amount': tax_amount * rate,
                    'round_off': round_off * rate,
                    'total': line_total * rate,
                    'paid': paid_amount * rate,
                    'balance': balance_amount * rate,
    ...
    def _get_invoice_data(self):
        """Fetch invoice data (vendor bills) with proper cash rounding handling"""
        invoice_data = []
        ...
        try:
            ...
            for invoice in invoices:
                ...
                # Get the total invoice cash rounding amount
                invoice_rounding_amount = self._get_invoice_cash_rounding_amount(invoice)

                # Count the number of invoice lines to distribute rounding
                invoice_lines = invoice.invoice_line_ids.filtered(
                    lambda l: l.display_type not in ['line_section', 'line_note']
                )

                # Filter lines based on analytic account if needed
                filtered_lines = []
                for line in invoice_lines:
                    ...
                    filtered_lines.append((line, warehouse_name, analytic_account))

                total_lines_count = len(filtered_lines)

                for idx, (line, warehouse_name, analytic_account) in enumerate(filtered_lines):
                    taxes = line.tax_ids
                    ...
                    # CORRECTED ROUND-OFF HANDLING:
                    # Distribute the total invoice rounding amount across lines
                    # Put all rounding on the last line to avoid rounding issues
                    round_off = 0
                    if total_lines_count > 0:
                        if idx == total_lines_count - 1:  # Last line gets all the rounding
                            round_off = invoice_rounding_amount
                        # Alternatively, distribute proportionally (uncomment if preferred):
                        # if invoice.amount_total > 0:
                        #     round_off = (line.price_total / invoice.amount_total) * invoice_rounding_amount
                    ...
                    # Get manual currency rate for this invoice
                    rate = self._get_manual_rate(invoice, self.company_id)
    ...
    def action_show_details(self):
        ...
        # Clear existing lines for this wizard instance
        self.env['purchase.register.line'].search([]).unlink()

        # Create line records
        line_ids = []
        for data in purchase_data:
            line = self.env['purchase.register.line'].create({
                'date': data.get('date'),
    ...
    def action_export_excel(self):
        ...
        headers = [
            ...
            'Warehouse', 'Product/Service', 'Quantity', 'Unit Price (SAR)', 'Subtotal (SAR)',
            'Trade Dis (SAR)', 'AddIn Dis (SAR)', 'AddIn Cost (SAR)', 'Tax', 'Tax Amount (SAR)',
            'Round Off (SAR)', 'Total (SAR)', 'Paid (SAR)', 'Balance (SAR)', 'Currency', 'Rate'
        ]
        ...
        for record in purchase_data:
            ...
            worksheet.write(row, 20, record.get('currency_rate', 1.0), currency_format)
...
#                         'invoice_id': invoice.id,  # Added for debugging
#                     })
#         except Exception as e:
#             _logger.error(f"Error fetching invoice data: {str(e)}")
#

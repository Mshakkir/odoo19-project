...
class ReportByCustomerWizard(models.TransientModel):
    ...
    def action_apply(self):
        ...
        if not self.show_all_customers:
            if not self.partner_ids:
                return {
                    ...
                    'params': {
                        'title': 'Warning',
                        'message': 'Please select at least one customer or check "All Customers"',
                        'type': 'warning',
                        'sticky': False,
                    }
                }
        ...
        if self.show_all_customers:
            ...
            report_name = f'Sales Report - {self.partner_ids[0].display_name}'
        else:
            report_name = f'Sales Report - {len(self.partner_ids)} Customers'

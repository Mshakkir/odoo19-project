...
class SaleOrder(models.Model):
    ...
    # ── NEW: Auto-fill currency + rate from partner when partner is selected ─
    @api.onchange('partner_id')
    def _onchange_partner_id_apply_currency(self):
        """
        When a customer is selected on the sale order, auto-populate
        sale_currency_id and manual_currency_rate from the partner's
        configured Sales Currency and Currency Rate (set on the partner form).
        Only applies if the partner has a sale_currency_id configured.
        """
        for order in self:
            ...
            # Apply partner's sales currency if set
            if commercial.sale_currency_id:
                order.sale_currency_id = commercial.sale_currency_id
                order.currency_id = commercial.sale_currency_id
                # Apply partner's manual rate if set, otherwise leave for
                # _onchange_currency_auto_fill_rate to compute from system rate
                if commercial.manual_currency_rate:
                    order.manual_currency_rate = commercial.manual_currency_rate

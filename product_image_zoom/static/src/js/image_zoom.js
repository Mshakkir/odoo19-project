...
//        var self = this;
//
//        // Apply zoom-in cursor to all product image fields
//        function applyZoomCursor() {
//            document.querySelectorAll(".o_field_image img").forEach(function(img) {
//                img.style.cursor = "zoom-in";
//            });
...
//        applyZoomCursor();
//
//        // Re-apply cursor after DOM changes (OWL re-renders)
//        this._pizMutationObs = new MutationObserver(function(mutations) {
//            for (var i = 0; i < mutations.length; i++) {
...
//        this._pizMutationObs.observe(document.body, { childList: true, subtree: true });
//
//        // Click handler - capture phase so we fire before Odoo handlers
//        this._pizClickHandler = function(e) {
//            var t = e.target;
//            if (!t || t.tagName !== "IMG") return;
//
//            // Check it's inside an o_field_image wrapper
//            var node = t.parentElement;
//            var inside = false;
//            for (var i = 0; i < 10; i++) {
//                if (!node) break;
...
//            e.preventDefault();
//
//            // Build full-resolution URL: /web/image/product.template/<id>/image_1920
//            var fullSrc = self._pizFullUrl(t);
//            self._pizOpenLightbox(fullSrc);
//        };
//
//        document.addEventListener("click", this._pizClickHandler, true);
//    },
...
//        var src = (imgEl.getAttribute("src") || imgEl.src || "").split("?")[0];
//
//        // Pattern: /web/image/MODEL/ID/FIELD  → replace FIELD with image_1920
//        var m = src.match(/(\/web\/image\/(?:[^/]+\/)\d+\/)([^/?#]*)/);
//        if (m) return m[1] + "image_1920";
//
//        // Fallback: read record id from model
//        var resId = this.model && this.model.root && this.model.root.resId;
//        if (resId) {
//            var model = this.props.resModel || "product.template";
//            return "/web/image/" + model + "/" + resId + "/image_1920";
//        }
//
//        // Last resort: use the thumbnail src as-is
//        return src;
//    },
//
//    _pizOpenLightbox(src) {
//        // ── build DOM ────────────────────────────────────────────────────────
//        var root = _el("div", "piz-root");
//
//        var stage = _el("div", "piz-stage");
//        root.appendChild(stage);
//
//        var wrap = _el("div", "piz-wrap");
//        stage.appendChild(wrap);
//
...
//        root.appendChild(closeBtn);
//
//        var hint = _el("div", "piz-hint");
//        hint.textContent = "Scroll to zoom  ·  Drag to pan  ·  Esc to close";
//        root.appendChild(hint);
//
//        var bar    = _el("div", "piz-bar");
//        var bOut   = _btn("\u2212");
//        var label  = _el("div", "piz-pct"); label.textContent = "100%";
//        var bIn    = _btn("+");
//        var bReset = _btn("\u21BA");
//        [bOut, label, bIn, bReset].forEach(function(n) { bar.appendChild(n); });
//        root.appendChild(bar);
...
//});
//
//// ── tiny DOM helpers (module-level, not on prototype) ─────────────────────────

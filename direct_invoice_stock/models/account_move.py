...
class AccountMove(models.Model):
    ...
    def action_post(self):
        """Post invoice and create delivery order for stockable products"""
        ...
        for move in self:
            ...
            # Get stockable product lines (GOODS)
            stock_lines = move.invoice_line_ids.filtered(
                lambda l: l.product_id
                and l.product_id.detailed_type == 'product'
                and l.quantity > 0
            )

            if not stock_lines:
                raise UserError(
                    _("Failed to create delivery order:\n"
                      "No stockable products found in this invoice.\n"
                      "Please add products with type 'Goods'.")
                )

            picking = move._create_delivery_order(stock_lines)
            picking.action_confirm()
            picking.action_assign()
    ...
    def _create_delivery_order(self, stock_lines):
        ...
        if not picking_type:
            raise UserError(_("No outgoing delivery type found."))

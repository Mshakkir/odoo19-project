...
#                     continue  # Odoo 19: skip suspense lines, handled by keep mode
#                 line = (
#                     self.env["account.move.line"]
...
class AccountBankStatementLine(models.Model):
    ...
    def _compute_tax_lines_for_manual(self, base_vals, reconcile_auxiliary_id):
        """
        ...
        taxes = self.manual_tax_ids
        amount = base_vals["amount"]
        currency = self.company_id.currency_id

        # compute_all expects a positive price; we track sign separately
        ...
        for tax_line in tax_results.get("taxes", []):
            tax_account_id = tax_line.get("account_id")
            ...
            line = {
                "reference": f"reconcile_auxiliary;{reconcile_auxiliary_id}",
                "id": False,
                "account_id": [tax_account.id, tax_account.display_name],
                "partner_id": base_vals.get("partner_id"),
                "date": fields.Date.to_string(self.date),
                "name": tax_line.get("name") or self.manual_name,
                "amount": tax_amount,
        ...
        updated_base.update(
            {
                ...
                # Keep tax_ids on the base line — the tax report module needs this
                # to display the "15%" badge and to attribute the base amount to the
                # correct tax. Duplicate tax lines are avoided because our separate
                # tax line entries each carry a tax_repartition_line_id, which tells
                # Odoo's tax computation that these lines are already finalised and
    ...
    def _onchange_manual_reconcile_vals(self):
        ...
        # Remove stale tax lines generated by a previous call for this reference
        data = [
            l for l in data
            if not (
                    l.get("_is_tax_line_for")
                    and l["_is_tax_line_for"] == self.manual_reference
            )
        ]

        reconcile_auxiliary_id = self.reconcile_data_info.get(
            "reconcile_auxiliary_id", 1
        )
        extra_tax_lines = []
        ...
        for line in data:
            if line["reference"] == self.manual_reference:
                if self._check_line_changed(line):
                    ...
                    # Preserve the reference so tax lines can refer back to it
                    line_vals["reference"] = line["reference"]
                    if line["kind"] == "liquidity":
                        self._update_move_partner()
                    # Compute and collect split tax lines (non-liquidity only)
                    if self.manual_tax_ids and line_vals["kind"] != "liquidity":
                        line_vals, extra_tax_lines, reconcile_auxiliary_id = (
                            self._compute_tax_lines_for_manual(
                                line_vals, reconcile_auxiliary_id
                            )
                        )
                    line.update(line_vals)
            if self.manual_line_id and self.manual_line_id.id == line.get(
        ...
        # Append computed tax lines after the base line
        new_data.extend(extra_tax_lines)
    ...
    def _reconcile_bank_line_edit(self, data):
        ...
        with move._check_balanced(container):
            ...
            # Separate data into base lines and their associated manual tax lines.
            # We collect each base line together with its tax lines and write them
            # to the move in a single create call so that Odoo sees the tax lines
            # already present and does NOT auto-generate duplicates.
            pending_base = None
            pending_tax_vals = []

            def _flush_pending():
                nonlocal pending_base, pending_tax_vals
                if pending_base is None:
                    return
                base_line_vals = self._reconcile_move_line_vals(pending_base)
                ...
                if pending_base.get("counterpart_line_ids"):
                    to_reconcile.append(
                        self.env["account.move.line"].browse(
                            pending_base.get("counterpart_line_ids")
                        )
                        + base_line
                    )
                pending_base = None
            ...
            for line_vals in data:
                if line_vals["kind"] == "liquidity":
                    continue
                computed_vals = self._reconcile_move_line_vals(line_vals)
                if computed_vals.pop("_is_manual_tax_line", False):
                    # Queue this tax line to be written with the preceding base line
                    pending_tax_vals.append(line_vals)
                else:
                    # New base line — flush the previous base+tax group first
                    _flush_pending()
                    pending_base = line_vals
            _flush_pending()
    ...
    def _reconcile_move_line_vals(self, line, move_id=False):
        ...
        if line.get("line_currency_id") and line["currency_id"] != line.get(
            ...
            vals["amount_currency"] = line["currency_amount"]
        # If this line has a tax_repartition_line_id it is already a finalised tax
        # line — flag it so _reconcile_bank_line_edit can write base + tax lines
        # together in one move.write() and prevent Odoo auto-generating duplicates.
        if line.get("tax_repartition_line_id"):
            vals["_is_manual_tax_line"] = True
        return vals

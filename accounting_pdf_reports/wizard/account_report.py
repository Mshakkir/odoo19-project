...
class AccountingReport(models.TransientModel):
    ...
    _description = "Accounting Report Wizard with Comparison and PDF"
    ...
    label_filter = fields.Char(
        string='Column Label',
        help="This label will be displayed on the report to show "
    ...
    debit_credit = fields.Boolean(
        ...
        help="This option allows you to get more details about "
             "how your balances are computed. "
             "Because it is space-consuming, we do not allow it "
    ...
    def _get_account_report(self):
        """Get report automatically based on active menu."""
        reports = []
        active_id = self.env.context.get('active_id')
        if active_id:
            menu = self.env['ir.ui.menu'].browse(active_id).name
            reports = self.env['account.financial.report'].search([('name', 'ilike', menu)])
        return reports[:1] or False

    # -------------------------------------------------------------------------
    # CONTEXT BUILDING FOR COMPARISON
    # -------------------------------------------------------------------------
    def _build_comparison_context(self, data):
        result = {}
        form = data.get('form', {})
        result['journal_ids'] = form.get('journal_ids', False)
        result['state'] = form.get('target_move', '')
        if form.get('filter_cmp') == 'filter_date':
            result['date_from'] = form.get('date_from_cmp')
            result['date_to'] = form.get('date_to_cmp')
            result['strict_range'] = True
        return result

    # -------------------------------------------------------------------------
    # CHECK REPORT (adds comparison context)
    # -------------------------------------------------------------------------
    def check_report(self):
        """Override to inject comparison data into report context."""
        ...
        # Handle Many2one tuples safely
        if isinstance(form_data.get('account_report_id'), tuple):
            form_data['account_report_id'] = form_data['account_report_id'][0]

        comparison_context = self._build_comparison_context({'form': form_data})
        res['data']['form']['comparison_context'] = comparison_context
        return res

    # -------------------------------------------------------------------------
    # PRINT REPORT ACTION (PDF)
    # -------------------------------------------------------------------------
    def _print_report(self, data):
        """Call the financial PDF report action."""

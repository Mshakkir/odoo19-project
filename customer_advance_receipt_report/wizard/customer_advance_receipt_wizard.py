...
class CustomerAdvanceReceiptWizard(models.TransientModel):
    ...
    def action_show_report(self):
        ...
        # Build domain - fetch advance customer receipts only
        domain = [
            ('date', '>=', self.date_from),
            ('date', '<=', self.date_to),
            ('is_advance_payment', '=', True),
            # Customer receipts are inbound payments
        ...
        _logger.info("Customer Advance Receipt Report: found %d receipts", len(payments))
        ...
        for receipt in payments:
            ...
            # Original amount in payment currency (as entered by user)
            receipt_amount = receipt.amount

            # Company currency amount using the manual exchange rate on the payment form.
            # manual_currency_exchange_rate: 1 [payment_currency] = X [company_currency]
            # When same currency, rate = 1.0 so formula is always safe.
            manual_rate = receipt.manual_currency_exchange_rate or 1.0
            amount_in_company_currency = receipt_amount * manual_rate

            _logger.info(
                "Receipt %s | %s %s | rate %s | company currency %s %s",
                receipt.name,
                receipt_amount,
                receipt.currency_id.name,
                manual_rate,

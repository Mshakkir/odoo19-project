...
class ReportTaxReport(models.AbstractModel):
    ...
    def _get_taxes(self, move_lines):
        """Compute tax totals grouped by tax"""
        tax_data = {}
        for line in move_lines:
            for tax_line in line.tax_ids:
                if tax_line.id not in tax_data:
                    tax_data[tax_line.id] = {
                        'name': tax_line.name,
                        'base': 0.0,
                        'amount': 0.0,
                    }
                tax_data[tax_line.id]['base'] += line.debit - line.credit
                tax_data[tax_line.id]['amount'] += tax_line.amount
        return tax_data
    ...
    def _get_report_data(self, analytic_account_ids, date_from, date_to, target_move):
        ...
        lines = move_line_obj.search(domain)
        return self._get_taxes(lines)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        taxes = self._get_report_data(
            analytic_accounts,
            form_data.get('date_from'),
            form_data.get('date_to'),
            form_data.get('target_move'),
        )

    ...
    def get_product_purchase_history(self, product_id):
        ...
        try:
            ...
            _logger.info("Search domain: [('product_id', '=', %s), ('move_id.move_type', '=', 'in_invoice')]",
                         product_id)
            _logger.info("Order by: move_id.invoice_date desc")
            ...
            _logger.info("Search completed successfully. Found %s lines", len(invoice_lines))
            ...
            for idx, line in enumerate(invoice_lines):
                ...
                try:
                    ...
                    _logger.info("  - Move State: %s", line.move_id.state if line.move_id else 'NO STATE')
                    ...
                    _logger.info("  - Order: %s, Partner: %s", order_name, partner_name)

                    # Get date
                    date_order = ''
                    if line.move_id.invoice_date:
                        date_order = line.move_id.invoice_date.strftime('%Y-%m-%d')
                    _logger.info("  - Date: %s", date_order)

                    # Get quantity and UOM
                    product_qty = line.quantity
                    product_uom = line.product_uom_id.name if line.product_uom_id else ''
                    _logger.info("  - Quantity: %s %s", product_qty, product_uom)

                    # Get prices
                    price_unit = line.price_unit
                    price_subtotal = line.price_subtotal
                    _logger.info("  - Price unit: %s, Subtotal: %s", price_unit, price_subtotal)
                    ...
                    _logger.exception("Full line error traceback:")
                    continue

            _logger.info("Processing complete. Total records: %s", len(history))
            _logger.info("Returning history data...")

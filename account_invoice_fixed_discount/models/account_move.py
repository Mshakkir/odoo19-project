...
class AccountMove(models.Model):
    ...
    global_discount_fixed = fields.Monetary(
        ...
        default=0.0,
        currency_field="currency_id",
        help="Apply a fixed discount to the entire invoice (without distributing across lines).",
        tracking=True,
        states={'posted': [('readonly', True)]},
    )
    ...
    @api.onchange('global_discount_fixed')
    def _onchange_global_discount_fixed(self):
        """Only trigger a total recompute; do not distribute discount to lines."""
        # Do nothing to invoice_line_ids
        # Only recalculate totals
        self._recompute_dynamic_lines(recompute_tax_base_amount=True)

    def _compute_amount(self):
        """Modify this compute to subtract the global discount from total."""
        super()._compute_amount()

        for move in self:
            if move.global_discount_fixed:
                move.amount_untaxed -= move.global_discount_fixed
                move.amount_total -= move.global_discount_fixed
                move.amount_residual -= move.global_discount_fixed

    ...
    def _prepare_stock_moves(self, picking):
        ...
        if self.warehouse_id and values:
            for move_vals in values:
                ...
                move_vals['location_dest_id'] = self.warehouse_id.lot_stock_id.id
                move_vals['warehouse_id'] = self.warehouse_id.id
                # Set source warehouse for tracking
                if self.order_id.picking_type_id and self.order_id.picking_type_id.warehouse_id:
                    move_vals['source_warehouse_id'] = self.order_id.picking_type_id.warehouse_id.id
...
class PurchaseOrder(models.Model):
    ...
    def _create_picking(self):
        ...
        if len(warehouses_in_order) <= 1:
            ...
            # Set source warehouse on created pickings
            for order in self:
                if order.picking_type_id and order.picking_type_id.warehouse_id:
                    source_wh = order.picking_type_id.warehouse_id
                    for picking in order.picking_ids.filtered(lambda p: not p.source_warehouse_id):
                        picking.source_warehouse_id = source_wh
            return result
        ...
        for order in self:
            if any(product.type in ['product', 'consu'] for product in order.order_line.mapped('product_id')):
                # Group lines by warehouse
                lines_by_warehouse = defaultdict(list)
                source_warehouse = order.picking_type_id.warehouse_id if order.picking_type_id else None
                ...
                for warehouse, lines in lines_by_warehouse.items():
                    ...
                    if existing_picking:
                        ...
                        picking_vals = {
                            ...
                            'location_id': order.partner_id.property_stock_supplier.id or self.env.ref(
                                'stock.stock_location_suppliers').id,
                            'company_id': order.company_id.id,
                            'source_warehouse_id': source_warehouse.id if source_warehouse else False,
                        }
                        picking = StockPicking.create(picking_vals)
                    ...
                    for line in lines:
                        moves = line._create_stock_moves(picking)
                        # Set source warehouse on moves
                        if source_warehouse:
                            moves.write({'source_warehouse_id': source_warehouse.id})
                        moves._action_confirm()
                        moves._action_assign()

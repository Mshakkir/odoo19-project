...
class AccountPartnerLedgerCustom(models.TransientModel):
    ...
    show_po_number = fields.Boolean(
        string='Show PO Number Column',
        default=False,
        help="If enabled, the PO Number column will be printed in the report.",
    )

    def check_report(self):
        res = super().check_report()
        # Inject show_po_number into the form data so the template can read it
        if res.get('data') and res['data'].get('form') is not None:
            res['data']['form']['show_po_number'] = self.show_po_number
        return res

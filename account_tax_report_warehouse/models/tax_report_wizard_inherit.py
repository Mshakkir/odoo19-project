...
#
#    # add now for details get warehouse base
...
#         tables, where_clause, where_params = self.env['account.move.line'].with_context(context)._query_get()
#
#         # ✅ Apply analytic (warehouse) filter
#         if self.analytic_account_ids:
#             analytic_filter = " AND (" + " OR ".join(
#                 [f"account_move_line.analytic_distribution ? '{aid}'" for aid in self.analytic_account_ids.ids]
#             ) + ")"
#             where_clause += analytic_filter
#
#         # --- 1️⃣ Compute Tax Amounts ---
#         sql = self._sql_from_amls_one()
#         query = sql % (tables, where_clause)
#         self.env.cr.execute(query, where_params)
#         results = self.env.cr.fetchall()
...
#         })
#
#     # --- Action Button to Open the Report ---
#     def open_tax_report_details(self):
#         self.compute_tax_summary()
#         return {
#             'name': 'Tax Report Summary',
...
# tax amount display in this code

...
class StockPickingInvoiceWizard(models.TransientModel):
    ...
    def action_create_invoice(self):
        ...
        # Collect delivery note names to populate the Delivery Note Number field
        picking_names = self.picking_ids.mapped('name')
        delivery_note_number = ', '.join(picking_names)

        invoice_vals = {
            'move_type': 'out_invoice',
            'partner_id': self.partner_id.id,
            'invoice_date': self.invoice_date,
            'invoice_origin': delivery_note_number,
            'journal_id': self.journal_id.id,
            'invoice_line_ids': [],
        }

        # Set the Delivery Note Number field if it exists on account.move (odoo-mates field)
        if 'delivery_note_number' in self.env['account.move']._fields:
            invoice_vals['delivery_note_number'] = delivery_note_number

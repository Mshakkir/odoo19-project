...
class AccountDaybookAnalyticWizard(models.TransientModel):
    ...
    def action_show_details(self):
        ...
        # Resolve the inherited list view so Currency and Balance columns are always visible
        list_view_id = self.env.ref(
            'account.view_move_line_tree', raise_if_not_found=False
        )
        ...
        return {
            ...
            'view_mode': 'list,form',
            'views': [(list_view_id.id if list_view_id else False, 'list'), (False, 'form')],
            'domain': final_domain,
            'target': 'current',
            'context': {
                'create': False,
                # Force optional columns on so Currency and Balance are always shown
                'list_view_optional_columns': ['currency_id', 'balance'],
            },
        }

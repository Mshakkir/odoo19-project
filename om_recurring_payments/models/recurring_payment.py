...
class RecurringPayment(models.Model):
    ...
    recurring_interval = fields.Integer(
        'Recurring Interval', required=True,
        related='template_id.recurring_interval', readonly=True
    )
    ...
    def compute_next_date(self, date_val):
        period = self.recurring_period
        interval = self.recurring_interval
        if period == 'days':
            return date_val + relativedelta(days=interval)
        elif period == 'weeks':
            return date_val + relativedelta(weeks=interval)
        elif period == 'months':
            return date_val + relativedelta(months=interval)
        else:
            return date_val + relativedelta(years=interval)

    def action_create_lines(self, date_val):
        vals = {
            'partner_id': self.partner_id.id,
            'amount': self.amount,
            'date': date_val,
        ...
        self.env['recurring.payment.line'].create(vals)

    def action_done(self):
        date_begin = self.date_begin
        while date_begin < self.date_end:
            self.action_create_lines(date_begin)
            date_begin = self.compute_next_date(date_begin)
        self.state = 'done'

    def action_draft(self):
        if self.line_ids.filtered(lambda t: t.state == 'done'):
            raise ValidationError(_('You cannot set to Draft as one of the lines is already in done state'))
    ...
    def create(self, vals_list):
        for vals in vals_list:
            seq = self.env['ir.sequence'].with_context(
                force_company=vals.get('company_id') or self.env.company.id
            )
            vals['name'] = seq.next_by_code('recurring.payment') or _('New')
    ...
    def _check_amount(self):
        for rec in self:
            if rec.amount <= 0:
                raise ValidationError(_('Amount must be a positive non-zero number'))
...
class RecurringPaymentLine(models.Model):
    ...
    recurring_payment_id = fields.Many2one('recurring.payment', string="Recurring Payment")
    partner_id = fields.Many2one('res.partner', 'Partner', required=True)
    amount = fields.Monetary('Amount', required=True, default=0.0, currency_field='currency_id')

...
class PurchaseProductReportNew(models.TransientModel):
    ...
    price_total = fields.Float(string='Net Amount', readonly=True)

    # Hidden columns
    discount_fixed = fields.Float(string='Discount (Fixed)', readonly=True)
    price_subtotal = fields.Float(string='Subtotal', readonly=True)
    tax_amount = fields.Float(string='Tax Value', readonly=True)
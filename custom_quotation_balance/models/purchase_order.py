...
class PurchaseOrder(models.Model):
    ...
    vendor_total_billed = fields.Monetary(
        ...
        compute='_compute_vendor_balance',
        currency_field='currency_id',
        help='Total of all vendor bills'
    )
        ...
        for order in self:
            if order.partner_id:
                try:
                    ...
                    order.vendor_total_paid = total_payments + total_credits
                    order.vendor_balance_due = total_billed - (total_payments + total_credits)

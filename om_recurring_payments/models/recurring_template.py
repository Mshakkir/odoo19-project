...
class AccountRecurringTemplate(models.Model):
    ...
    next_call = fields.Date(string="Next Execution Date", compute="_compute_next_call", store=True)
    ...
    @api.depends('date_begin', 'date_end', 'recurring_period', 'recurring_interval')
    def _compute_next_call(self):
        for rec in self:
            if rec.date_begin and rec.date_end:
                if rec.recurring_period == 'days':
                    ...
                    exec_date = rec.date_begin + relativedelta(weeks=rec.recurring_interval)
                elif rec.recurring_period == 'months':
                    exec_date = rec.date_begin + relativedelta(months=rec.recurring_interval)
                else:
                    exec_date = rec.date_begin + relativedelta(years=rec.recurring_interval)

                rec.next_call = exec_date if exec_date <= rec.date_end else False
            else:
                rec.next_call = False

...
class AccountMove(models.Model):
    ...
    # PO Number field - shows ALL related POs (computed from invoice lines)
    po_number = fields.Many2many(
        ...
        store=True,
        readonly=False,
        help='All Purchase Orders linked to this invoice'
    )
    ...
    def _onchange_partner_id(self):
        """Clear GR and other fields when vendor changes"""
    ...
    def _onchange_purchase_auto_complete(self):
        ...
        if purchase_order:
            ...
            # Find related Goods Receipt (incoming picking) - only if not set
    ...
    def _onchange_invoice_lines(self):
        """Recompute PO numbers when invoice lines change in the UI"""
    ...
    def _onchange_goods_receipt(self):
        ...
        if self.goods_receipt_number:
            ...
            if gr.purchase_id:
                ...
                if not self.buyer_id and po.user_id:
                    self.buyer_id = po.user_id

                # Get Deliver To from the related PO
                if not self.deliver_to and po.picking_type_id:
                    self.deliver_to = po.picking_type_id

                # Also get AWB from the related PO
                if not self.awb_number and hasattr(po, 'awb_number') and po.awb_number:

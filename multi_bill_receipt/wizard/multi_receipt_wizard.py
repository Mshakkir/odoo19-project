...
#             }
#
#         wizard = self.env['receipt.list.display.wizard'].create({
#             'partner_id': self.vendor_id.id,
...
#             'type': 'ir.actions.act_window',
#             'res_model': 'receipt.list.display.wizard',
#             'res_id': wizard.id,
#             'view_mode': 'form',
#             'target': 'new',
#         }
...
#                 'tag': 'display_notification',
#                 'params': {
#                     'title': _('No Unpaid Bills'),
#                     'message': _('All bills for this vendor are paid.'),
#                     'type': 'info',
#                 }
#             }
#
...
#         _logger.info(f"Total bill lines: {len(self.invoice_line_ids)}")
#         for line in self.invoice_line_ids:
#             _logger.info(f"Line ID: {line.id}, Bill ID: {line.invoice_id.id if line.invoice_id else 'None'}, "
#                          f"Bill Number: {line.invoice_number}, Selected: {line.selected}, Amount: {line.amount_to_pay}")
#
#         bills_to_pay = []
...
#
#             raise UserError(_(
#                 'Please enter amounts to pay for selected bills.\n\n'
#                 'Selected bills and their amounts:\n%s\n\n'
...
#             ) % '\n'.join(error_details))
#
#         total_allocated = sum(item['amount'] for item in bills_to_pay)
#
#         if total_allocated > self.receipt_amount:
#             raise ValidationError(_('Total allocated amount (%.2f) cannot exceed receipt amount (%.2f)')
...
#             else:
#                 payment_vals['payment_reference'] = _('Payment for %s', bill.name)
#

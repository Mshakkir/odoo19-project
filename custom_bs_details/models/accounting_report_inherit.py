...
class AccountingReportInherit(models.TransientModel):
    _inherit = 'accounting.report'  # Inherit your Odoomates balance sheet wizard
    ...
    def action_show_details(self):
        ...
        Excludes VAT Input, VAT Output, and Profit/Loss accounts.
        """
        self.ensure_one()
        ...
        # ✅ Build WHERE conditions from domain filters
        ...
        for row in rows:
            acc = Account.browse(row.get('account_id'))
            BalanceLine.create({
                'wizard_id': self.id,
                'account_id': acc.id,

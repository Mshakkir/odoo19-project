...
class AccountPayment(models.Model):
    ...
    # Date displayed as dd/mm/yy in list view.
    # A computed Char field is the only reliable way to enforce a fixed date
    # format in Odoo 17+/19 list views, since the Date widget respects the
    # user's language locale and ignores widget options.
    ...
    def _compute_date_display(self):
        for payment in self:
            if payment.date:
                payment.date_display = payment.date.strftime('%d/%m/%y')
            else:
                payment.date_display = ''

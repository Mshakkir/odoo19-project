...
class PartnerLedgerDetail(models.TransientModel):
    ...
    # Exchange rate used (per-line, from payment or wizard override)
    manual_currency_exchange_rate = fields.Float(
        string='Exchange Rate', digits=(12, 6), readonly=True, default=1.0,
    )
    ...
    # ── Helper: resolve effective exchange rate for a move line ─────────────
    def _get_effective_rate(self, line, wizard_use_manual, wizard_rate, wizard_currency_id):
        """
        Priority:
          1. If wizard has use_manual_rate=True AND the line's currency matches
             the wizard currency  →  use wizard rate.
          2. Else fall back to rate stored on linked account.payment.
          3. Else 1.0 (no conversion).
        """
        company_currency = self.env.company.currency_id

        # Wizard-level override
        if wizard_use_manual and wizard_currency_id and wizard_rate:
            if line.currency_id and line.currency_id.id == wizard_currency_id:
                return wizard_rate
        ...
        if payment and hasattr(payment, 'manual_currency_exchange_rate') \
                and payment.manual_currency_exchange_rate:
            return payment.manual_currency_exchange_rate
    ...
    def get_partner_ledger_details(self, wizard_data):
        ...
        result_selection = wizard_data.get('result_selection', 'customer')
        reconciled = wizard_data.get('reconciled', False)
        show_amount_currency = wizard_data.get('amount_currency', False)

        # Wizard-level manual rate
        wizard_use_manual = wizard_data.get('use_manual_rate', False)
        wizard_currency_id = wizard_data.get('manual_rate_currency_id', False)
        wizard_rate = wizard_data.get('manual_currency_exchange_rate', 1.0)
        ...
        for partner in move_lines.mapped('partner_id'):
            ...
            if date_from:
                opening_balance = self._calculate_opening_balance(
                    partner, date_from, account_ids, move_state, reconciled,
                    journal_ids, wizard_use_manual, wizard_currency_id, wizard_rate
                )
                if opening_balance != 0:
                    detail_records.append({
                        'partner_id': partner.id,
            ...
            for line in partner_lines:
                effective_rate = self._get_effective_rate(
                    line, wizard_use_manual, wizard_rate, wizard_currency_id
                )
                ...
                if is_foreign:
                    raw_amount = abs(line.amount_currency)
                    converted = raw_amount * effective_rate
                    debit_val = converted if line.amount_currency > 0 else 0.0
                    credit_val = converted if line.amount_currency < 0 else 0.0
                else:
                    debit_val = line.debit
                ...
                vals = {
                    ...
                    'invoice_date_due': line.move_id.invoice_date_due or False,
                    'po_number': line.move_id.client_order_ref or '',
                    'manual_currency_exchange_rate': effective_rate,
                }

                if show_amount_currency and line.currency_id and line.currency_id != company_currency:
                    vals.update({
                        'amount_currency': line.amount_currency,
                        'currency_id': line.currency_id.id,
                    })
    ...
    def _calculate_opening_balance(
        self, partner, date_from, account_ids, move_state, reconciled,
        journal_ids, wizard_use_manual=False, wizard_currency_id=False, wizard_rate=1.0
    ):
        domain = [
            ('partner_id', '=', partner.id),
            ('account_id', 'in', account_ids),
        ...
        for line in lines:
            ...
            if is_foreign:
                ...
                total += converted if line.amount_currency > 0 else -converted
            else:

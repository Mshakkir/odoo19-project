...
registry.category("services").add("stock_ledger_filter", {
    ...
    start(env, { orm, notification }) {
        ...
        // ── Check if current view is the Stock Ledger ─────────────────
        function isStockLedgerView() {
            ...
            return title.includes("stock ledger");
        }

        // ── Find OWL list model from DOM ──────────────────────────────
        function getListModel() {
            ...
            while (el) {
                if (el.__owl__?.component?.model) {
                    return el.__owl__.component.model;
                }
                el = el.firstElementChild;
            }
            // Try parent chain too
            el = listView.parentElement;
        ...
        function applyDomain(domain) {
            ...
            if (model && model.root) {
                ...
                console.warn("[StockLedgerFilter] List model not found");
            }
        }

        // ── Build and inject the filter bar ──────────────────────────
        function buildBar() {
            if (document.querySelector(".o_sl_filter_bar")) return; // already exists
            ...
            bar.innerHTML = `
                <div class="slf-wrap">
                    ...
                    <div class="slf-field slf-date-wrap">
            ...
            // Insert as first child of list view
            listView.insertBefore(bar, listView.firstChild);
            ...
            const onKey = e => {
                ...
                // Only fire if focus is inside the filter bar or not inside any input
            ...
            // Store cleanup on bar element
            bar._cleanup = () => document.removeEventListener("keydown", onKey);
        }

        // ── MutationObserver: watch for ledger view ───────────────────
        const observer = new MutationObserver(() => {
            ...
            debounce = setTimeout(() => {
                ...
                // Only build if the list table is ready and bar not present

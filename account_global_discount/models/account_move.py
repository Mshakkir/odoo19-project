...
class AccountMove(models.Model):
    ...
    def _prepare_global_discount_vals(self, global_discount, base, tax_ids):
        """Prepare the dictionary values for an invoice global discount line."""
        self.ensure_one()
    ...
    def _set_global_discounts_by_tax(self):
        """Create invoice global discount lines by taxes combinations and discounts.
        ...
        # Build tax combinations and their bases
        _self = self.filtered("global_discount_ids")
        for inv_line in _self.invoice_line_ids.filtered(
                lambda line: line.display_type not in ["line_section", "line_note"]
        ):
            if inv_line.product_id and inv_line.product_id.bypass_global_discount:
                continue
        ...
        self.invoice_global_discount_ids.unlink()
    ...
    def _set_global_discounts_by_zero_tax(self, taxes_keys, create_method):
        """Handle global discounts for lines with zero-rate taxes."""
        base_total = 0
    ...
    def _compute_global_discount_ids(self):
        """Compute global discounts from partner configuration."""
        for move in self:
            ...
            for discount in move_discounts:
                if not discount.company_id or discount.company_id == move.company_id:
                    discounts |= discount
    ...
    def _clean_global_discount_lines(self):
        """Remove global discount lines from move."""
        self.ensure_one()
        gbl_disc_lines = self.line_ids.filtered(
            lambda l: l.invoice_global_discount_id
        )
        if gbl_disc_lines:
    ...
    def write(self, vals):
        """Recompute global discounts when invoice lines or discounts change."""
        ...
        if "invoice_line_ids" in vals or "global_discount_ids" in vals:
            for move in self:
                ...
                move._set_global_discounts_by_tax()
                move._recompute_global_discount_lines()
            self._global_discount_check()
        return res

    def _global_discount_check(self):
        """Validate tax configuration for global discounts."""

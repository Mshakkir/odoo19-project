...
class ReportTax(models.AbstractModel):
    ...
    def _compute_from_amls(self, options, taxes):
        ...
        # 🔹 Filter by analytic account of selected warehouse
        if warehouse_id:
            warehouse = self.env['stock.warehouse'].browse(warehouse_id)
            analytic_account = warehouse.analytic_account_id
            if analytic_account:
                where_clause += ' AND account_move_line.analytic_account_id = %s'
                where_params.append(analytic_account.id)

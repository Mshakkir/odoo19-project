...
class BillAllocationDisplayWizard(models.TransientModel):
    """Wizard to display bill allocation history for vendor payments"""
    _name = 'bill.allocation.display.wizard'
    _description = 'Bill Allocation Display'

    payment_id = fields.Many2one('account.payment', string='Payment', readonly=True)
    partner_id = fields.Many2one('res.partner', string='Vendor', related='payment_id.partner_id', readonly=True)
    ...
    allocation_line_ids = fields.One2many('bill.allocation.display.line', 'wizard_id', string='Bill Allocations')
...
class BillAllocationDisplayLine(models.TransientModel):
    """Transient model to display bill allocation lines in wizard"""

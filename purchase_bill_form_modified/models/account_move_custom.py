...
class AccountMove(models.Model):
    ...
    # Override partner_id with custom domain for vendor bills
    ...
    def _onchange_move_type_filter_partners(self):
        """Update partner domain based on move type"""
        if self.move_type in ['in_invoice', 'in_refund', 'in_receipt']:
            # Vendor bills - show only suppliers
            return {
                'domain': {
                    'partner_id': [
                        ('type', '!=', 'private'),
                        '|', ('company_id', '=', False), ('company_id', '=', self.company_id.id or self.env.company.id),
                        '|', ('supplier_rank', '>', 0), ('is_company', '=', True)
                    ]
                }
            }
        else:
            # Customer invoices - show all partners
            return {
                'domain': {
                    'partner_id': [
                        ('type', '!=', 'private'),

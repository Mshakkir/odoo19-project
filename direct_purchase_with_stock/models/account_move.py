...
class AccountMove(models.Model):
    ...
    def button_cancel(self):
        ...
        for bill in self:
            # ONLY handle vendor transactions
            if bill.move_type not in ['in_invoice', 'in_refund']:
                continue
    ...
    def _create_receipt_from_bill(self):
        ...
        _logger.info(f"🔵 PURCHASE: Starting receipt creation for bill: {self.name}")
        ...
        # For receipts: Vendor location → Warehouse (ADDS STOCK)
        location_dest_id = warehouse.lot_stock_id.id
        ...
        _logger.info(f"   📦 FROM: {vendor_location.complete_name} → TO: {warehouse.lot_stock_id.complete_name}")
    ...
    def _create_vendor_return_from_refund(self):
        """
        🔴 VENDOR RETURN: Return goods TO vendor (REMOVES stock from warehouse)

        Flow: Warehouse Stock → Vendors
        This DECREASES inventory
        """
        self.ensure_one()

        _logger.info(f"🔴 PURCHASE: Starting VENDOR return for refund {self.name}")
        _logger.info(f"   Move type: {self.move_type} (must be 'in_refund')")
        ...
        self._check_stock_availability(stockable_lines, warehouse)

        # ═══════════════════════════════════════════════════════════════
        # CRITICAL: VENDOR RETURN LOCATIONS
        # ═══════════════════════════════════════════════════════════════

        # Source: OUR warehouse (where we're taking stock FROM)
        location_id = warehouse.lot_stock_id.id
        _logger.info(f"   📍 FROM (source): {warehouse.lot_stock_id.complete_name}")

        # Destination: VENDOR location (where we're sending stock TO)
        vendor_location = self.env.ref('stock.stock_location_suppliers', raise_if_not_found=False)
        if not vendor_location:
        ...
        location_dest_id = vendor_location.id
        _logger.info(f"   📍 TO (destination): {vendor_location.complete_name}")

        # Use OUTGOING type for vendor returns (goods leaving warehouse)
        picking_type = warehouse.out_type_id
        if not picking_type:
            raise UserError(_('Delivery Orders operation type not found in warehouse %s') % warehouse.name)

        _logger.info(f"   📦 Picking type: {picking_type.name} (code: {picking_type.code})")
        _logger.info(f"   🔴 This will REMOVE stock from warehouse")

        # Create return picking
        picking_vals = {
            'picking_type_id': picking_type.id,
            'partner_id': self.partner_id.id,
            'origin': f"{self.name} (Return to Vendor)",
            'location_id': location_id,  # FROM warehouse
            'location_dest_id': location_dest_id,  # TO vendor
            'move_type': 'direct',
            'company_id': self.company_id.id,
        }
        ...
        # Link and notify
        self.receipt_id = picking.id
        message = _('✅ Vendor Return %s created - Products returned FROM warehouse TO vendor\n'
                    'Warehouse: %s\n'
                    'Stock DECREASED by return') % (picking.name, warehouse.name)
        self.message_post(body=message)

        _logger.info(f"   🔴 VENDOR RETURN completed - Stock REMOVED from {warehouse.name}")
        return picking

    def _get_warehouse(self):
        """Get warehouse for stock operations"""
        if self.purchase_warehouse_id:
            return self.purchase_warehouse_id
    ...
    def _check_stock_availability(self, stockable_lines, warehouse):
        """Check if enough stock is available for return"""
        stock_warnings = []
        is_stock_manager = self.env.user.has_group('stock.group_stock_manager')
        ...
        if stock_warnings:
            if not is_stock_manager:
                ...
                _logger.warning(f"Negative stock allowed by manager for refund {self.name}")

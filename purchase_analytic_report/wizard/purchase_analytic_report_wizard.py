...
class PurchaseAnalyticReportWizard(models.TransientModel):
    ...
    def _onchange_all_analytics(self):
        """When All Analytic Accounts is checked, clear analytic selection"""
    ...
    def action_show_report(self):
        ...
        # Validate: either all_analytics is checked OR analytics are selected
        if not self.all_analytics and not self.analytic_account_ids:
            raise models.ValidationError('Please select analytic accounts or check "All Analytic Accounts"')
        ...
        for invoice in invoices:
            ...
            for line in invoice.invoice_line_ids:
                if line.product_id and line.analytic_distribution:
                    ...
                    # If "All Analytics" is selected, use all analytics from the line
                    if self.all_analytics:
                        matching_analytics = analytic_account_ids
                    else:

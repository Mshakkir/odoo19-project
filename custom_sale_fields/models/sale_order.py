...
class SaleOrder(models.Model):
    ...
    def _get_order_currency(self):
        """Get effective currency: from pricelist if set, else from currency_id"""
        self.ensure_one()
        if self.pricelist_id and self.pricelist_id.currency_id:
            return self.pricelist_id.currency_id
        return self.currency_id

    @api.depends('currency_id', 'pricelist_id', 'company_id')
    def _compute_currency_rate_affixes(self):
        for order in self:
            company_currency = order.company_id.currency_id
            order_currency = order._get_order_currency()
    ...
    @api.depends('amount_total', 'manual_currency_rate', 'currency_id', 'pricelist_id', 'company_id')
    ...
    def _get_system_rate(self, order_currency, company_id, rate_date):
        ...
        unit_per_company = rate.get(order_currency.id, 1.0)
        return (1.0 / unit_per_company) if unit_per_company else 1.0

    @api.onchange('currency_id', 'pricelist_id', 'date_order')

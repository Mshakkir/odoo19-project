...
class ProductStockLedgerLine(models.Model):
    ...
    def _get_warehouse_from_location(self, location):
        """Get warehouse from location - Multiple methods"""
        if not location:
            return False

        # Method 1: Direct warehouse_id on location
        if hasattr(location, 'warehouse_id') and location.warehouse_id:
            return location.warehouse_id

        # Method 2: Get all warehouses and check if location is child
        warehouses = self.env['stock.warehouse'].search([])
        for wh in warehouses:
            if wh.view_location_id:
                # Check if location is under warehouse's view location
                if location.id == wh.view_location_id.id:
                    return wh
                # Check parent locations
                parent = location
                while parent:
                    if parent.id == wh.view_location_id.id:
                        return wh
                    parent = parent.location_id if parent.location_id else False
    ...
    def generate_ledger(self, product_id=None, warehouse_id=None, date_from=None, date_to=None):
        ...
        for mv in moves:
            ...
            if not move_warehouse:
                # For incoming moves, check destination
                if mv.location_dest_id and mv.location_dest_id.usage == 'internal':
                    move_warehouse = self._get_warehouse_from_location(mv.location_dest_id)

                # For outgoing moves, check source
                if not move_warehouse and mv.location_id and mv.location_id.usage == 'internal':
                    move_warehouse = self._get_warehouse_from_location(mv.location_id)

            # Method 5: Check both locations if still not found
            if not move_warehouse and mv.location_dest_id:
                move_warehouse = self._get_warehouse_from_location(mv.location_dest_id)
            if not move_warehouse and mv.location_id:
                move_warehouse = self._get_warehouse_from_location(mv.location_id)

            # Use filtered warehouse if specified
            if warehouse_id:
                move_warehouse = wh
            ...
            lines_to_create.append({
                'product_id': mv.product_id.id,
                'warehouse_id': move_warehouse_id,  # ✅ Enhanced detection
                'date': mv.date,
